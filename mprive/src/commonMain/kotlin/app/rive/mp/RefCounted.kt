...
interface RefCounted : CheckableAutoCloseable {
    ...
    /**
     * Close this object, releasing the reference.
     * Part of the [AutoCloseable] interface for use with `use` blocks.
     */
    override fun close() {
        release("AutoCloseable", "close() called")
    }
}
...
class RCPointer(
    ...
    private val referenceCount = atomic(1)
    ...
    val pointer: Long
        get() {
            check(referenceCount.value > 0) { "Attempting to access a disposed RCPointer ($label)" }
            return cppPointer
        }
    ...
    override fun acquire(source: String) {
        ...
        while (true) {
            val current = referenceCount.value
    ...
    override fun release(source: String, reason: String) {
        ...
        val currentCount = referenceCount.value
        RiveLog.v(TAG) {
            "Releasing $label (source: $source$reasonLog; " +
                    "ref count before release: $currentCount)"
        }
